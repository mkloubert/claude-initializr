...
- **侧边栏导航**：使用可折叠侧边栏快速访问所有配置部分
- **分割窗格布局**：编辑器和实时预览并排显示，支持调整大小
- **RTL 语言支持**：阿拉伯语、希伯来语和乌尔都语完全支持从右到左（RTL）方向

### 通用功能

- **实时预览**：在分割窗格中实时查看生成的配置文件预览
...
| `Ctrl/⌘ + 1-5` | 切换到部分（1=Dockerfile、2=Docker Compose、3=CLAUDE.md、4=settings.json、5=DevContainer） |
| `Ctrl/⌘ + /` | 打开键盘快捷键帮助 |
| `Escape` | 关闭对话框 |

通过标题栏中的键盘快捷键图标访问键盘快捷键帮助。
...
- **主要 UI/UX 重新设计**：完整的界面改进
  ...
  - 侧边栏导航，支持通过键盘快捷键（`Ctrl/⌘ + 1-5`）快速切换部分
  - 可调整大小的分割窗格，编辑器和实时预览并排显示
  - 可折叠的侧边栏和纯图标模式（`Ctrl/⌘ + B` 切换）
  - 使用手风琴部分的渐进式信息展示，减少认知负担
- **RTL 语言支持**：阿拉伯语、希伯来语和乌尔都语完全支持从右到左
  - RTL 语言的侧边栏自动定位在右侧
  - 所有 UI 元素正确镜像显示
- **响应式改进**：
  - 平板电脑屏幕（768-1023px）上侧边栏以折叠状态开始
  - 移动设备优化布局和底部工作表预览
  - 触摸友好的手风琴触发器（最小高度 44px）
- **欢迎对话框**：首次访问时向用户显示欢迎对话框
  - 可通过侧边栏的"关于"重新打开
- **删除旧版组件**：清理旧的卡片式组件
- 更新键盘快捷键以适应新布局
