...
- **사이드바 네비게이션**: 축소 가능한 사이드바로 모든 구성 섹션에 빠르게 접근
- **분할 창 레이아웃**: 에디터와 라이브 미리보기를 나란히 표시, 크기 조절 가능
- **RTL 언어 지원**: 아랍어, 히브리어, 우르두어에 대한 완전한 오른쪽에서 왼쪽 지원

### 일반 기능

- **실시간 미리보기**: 생성된 구성 파일의 실시간 미리보기를 분할 창에서 확인
...
| `Ctrl/⌘ + 1-5` | 섹션으로 이동 (1=Dockerfile, 2=Docker Compose, 3=CLAUDE.md, 4=settings.json, 5=DevContainer) |
| `Ctrl/⌘ + /` | 키보드 단축키 도움말 열기 |
| `Escape` | 대화 상자 닫기 |

헤더의 키보드 단축키 아이콘으로 도움말을 표시할 수 있습니다.
...
- **메이저 UI/UX 리디자인**: 완전한 인터페이스 오버홀
  ...
  - 키보드 단축키(`Ctrl/⌘ + 1-5`)를 사용한 빠른 섹션 전환 기능이 있는 사이드바 네비게이션
  - 에디터와 라이브 미리보기를 나란히 표시하는 크기 조절 가능한 분할 창
  - 축소 가능한 사이드바와 아이콘 전용 모드(`Ctrl/⌘ + B`로 전환)
  - 인지 부하를 줄이기 위한 아코디언 섹션을 사용한 점진적 공개
- **RTL 언어 지원**: 아랍어, 히브리어, 우르두어에 대한 완전한 오른쪽에서 왼쪽 지원
  - RTL 언어의 경우 사이드바가 자동으로 오른쪽에 배치
  - 모든 UI 요소가 적절하게 미러링됨
- **반응형 개선 사항**:
  - 태블릿 화면(768~1023px)에서 사이드바가 축소된 상태로 시작
  - 모바일 최적화 레이아웃 및 하단 시트 미리보기
  - 터치 친화적인 아코디언 트리거(최소 높이 44px)
- **시작 대화상자**: 처음 방문하는 사용자를 위한 시작 대화상자
  - 사이드바의 "정보"에서 다시 표시 가능
- **레거시 컴포넌트 제거**: 이전 카드 기반 컴포넌트 정리
- 새 레이아웃에 맞게 키보드 단축키 업데이트
