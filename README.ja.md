...
- **ソフトウェア選択**：インストールする追加ソフトウェアを選択：
  ...
  - Ollama（オープンソースモデルを使用するためにローカルOllamaインスタンスに接続）
...
- **サイドバーナビゲーション**：折りたたみ可能なサイドバーで、すべての設定セクションにすぐアクセス
- **分割ペインレイアウト**：エディターとライブプレビューを左右に並べて表示、リサイズ可能
- **RTL言語サポート**：アラビア語、ヘブライ語、ウルドゥー語に完全な右から左（RTL）対応

### 一般機能

- **ライブプレビュー**：生成された設定ファイルのリアルタイムプレビューを分割ペインで表示
...
- **設定履歴**: 元に戻す/やり直し機能で変更を追跡
  - 遅延スナップショットによる自動変更追跡
  - キーボードショートカットで元に戻す/やり直し（`Ctrl+Z` / `Ctrl+Y`）
  - タイムスタンプと変更説明付きの履歴パネル
  - 任意の状態を比較するdiffビュー
  - 以前の設定状態への復元
  - 永続化のためIndexedDBに保存（最大50エントリ）
...
| `Ctrl/⌘ + 1-5` | セクションに移動（1=Dockerfile、2=Docker Compose、3=CLAUDE.md、4=settings.json、5=DevContainer） |
| `Ctrl/⌘ + /` | キーボードショートカットヘルプを開く |
| `Escape` | ダイアログを閉じる |

ヘッダーのキーボードショートカットアイコンからショートカットヘルプを表示できます。
...
- ソフトウェアパッケージとしてOllamaサポートを追加
  ...
  - 有効時に`ANTHROPIC_BASE_URL`と`ANTHROPIC_AUTH_TOKEN`を自動設定
  - Ollama無効時に環境変数を削除
- ホストネットワークアクセスのためにdocker-compose.yamlに`extra_hosts`設定を追加

### v4.0.2

- **メジャーUI/UXリデザイン**: インターフェースの完全なオーバーホール
  - 縦型カードベースのレイアウトをサイドバー + 分割ペインレイアウトに置き換え
  - キーボードショートカット（`Ctrl/⌘ + 1-5`）によるクイックセクション切り替え機能付きサイドバーナビゲーション
  - リサイズ可能な分割ペイン、エディターとライブプレビューを左右に並べて表示
  - 折りたたみ可能なサイドバーとアイコンのみモード（`Ctrl/⌘ + B`で切り替え）
  - 認知負荷を減らすためのアコーディオンセクションによるプログレッシブディスクロージャー
- **RTL言語サポート**: アラビア語、ヘブライ語、ウルドゥー語に完全な右から左対応
  - RTL言語ではサイドバーが自動的に右側に配置
  - すべてのUI要素が適切にミラーリング
- **レスポンシブ改善**:
  - タブレット画面（768～1023px）ではサイドバーが折りたたまれた状態で開始
  - モバイル最適化レイアウトとボトムシートプレビュー
  - タッチフレンドリーなアコーディオントリガー（最小44pxの高さ）
- **ウェルカムダイアログ**: 初回訪問時にウェルカムダイアログを表示
  - サイドバーの「概要」から再度表示可能
- **レガシーコンポーネントの削除**: 古いカードベースコンポーネントの削除
- 新しいレイアウトに合わせてキーボードショートカットを更新
...
- 元に戻す/やり直し機能を備えた設定履歴を追加
  ...
  - 設定を比較するdiffビュー
  - 以前の状態への復元
  - 永続化のためのIndexedDBストレージ（最大50エントリ）
- VS CodeとGitHub CodespacesのDevContainerサポートを追加
  - `devcontainer.json`設定の生成
  - VS Code拡張機能と設定の構成
  - Dev Container機能の追加
  - ポートフォワーディングの設定
  - ライフサイクルコマンドの設定（post-create、post-start、post-attach）
  - 選択したソフトウェアに基づく自動拡張機能推奨
- DevContainerカードへスクロールするキーボードショートカット `Ctrl/⌘ + 5` を追加
- DevContainer機能でウェルカムセクションを更新
